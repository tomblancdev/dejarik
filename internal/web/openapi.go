...
func (s *Server) openapi(w http.ResponseWriter, _ *http.Request) {
  ...
  "components": {
    ...
    "schemas": {
      ...
      "Shelf": {"type": "object", "description": "One system's folder in the house store.", "properties": {"system": {"type": "string"}, "label": {"type": "string"}, "titles": {"type": "integer"}, "bytes": {"type": "integer"}, "known": {"type": "boolean", "description": "a system ES-DE names (an unknown folder is shown, nothing can be pushed into it)"}}},
      "System": {"type": "object", "description": "A shelf ES-DE knows: its folder, its name on the menu, the extensions it takes.", "properties": {"name": {"type": "string"}, "label": {"type": "string"}, "extensions": {"type": "array", "items": {"type": "string"}}}},
      "Push": {"type": "object", "description": "What a push answered, file by file.", "properties": {"added": {"type": "array", "items": {"type": "object", "properties": {"system": {"type": "string"}, "file": {"type": "string"}, "bytes": {"type": "integer"}}}}, "refused": {"type": "array", "items": {"type": "object", "properties": {"file": {"type": "string"}, "why": {"type": "string"}}}}}},
      "Link": {
        "type": "object",
      ...
      "LinkStatus": {
        "type": "object",
        "description": "One drawer's link to an external account (a device beside its seats needs it — Le Juke's Spotify). The grant lives with the person at the identity gateway; this is whether it exists.",
        "properties": {"linked": {"type": "boolean"}, "since": {"type": "string", "format": "date-time"}, "by": {"type": "string", "description": "Who tapped."}}
      },
      "Link": {
        "type": "object",
        "properties": {"link": {"type": "string", "description": "The companion's name on the appliance (spotify)."}, "label": {"type": "string"}, "kind": {"type": "string"}, "drawer": {"type": "string"}, "drawer_label": {"type": "string"}, "shared": {"type": "boolean"}, "mine": {"type": "boolean"}, "status": {"$ref": "#/components/schemas/LinkStatus"}, "word": {"type": "string", "enum": ["linked", "not linked"]}, "since": {"type": "string"}}
      },
  ...
  "paths": {
    ...
    "/api/projects/{name}/links/sync": {"post": {"summary": "The appliance asks who is linked, per link. From the appliance's addresses only.", "description": "Every few seconds the appliance's watcher asks which drawers hold a grant for each companion, and starts one beside every seat of those drawers. The Foyer's sources are the lock; the vhost never reaches this.", "security": [], "parameters": [{"name": "name", "in": "path", "required": true, "schema": {"type": "string"}}], "responses": {"200": {"description": "who is linked", "content": {"application/json": {"schema": {"type": "object", "properties": {"linked": {"type": "object", "additionalProperties": {"type": "array", "items": {"type": "string"}}, "description": "companion name -> the drawers that hold a grant"}}}}}}, "403": {"description": "not the appliance"}, "404": {"description": "no such project, or nothing to link on it"}}}},
    "/api/projects/{name}/links/{sidecar}/token": {"get": {"summary": "The appliance asks an access token for a drawer's link. From the appliance's addresses only.", "description": "The panel is the broker: the grant (the provider's refresh token) lives with the person at the identity gateway; this mints an hour's access token from it — cached, refreshed before it expires, the rotated grant written back — and hands it to the companion beside the seat. Never logged.", "security": [], "parameters": [{"name": "name", "in": "path", "required": true, "schema": {"type": "string"}}, {"name": "sidecar", "in": "path", "required": true, "schema": {"type": "string"}}, {"name": "drawer", "in": "query", "required": true, "schema": {"type": "string"}}], "responses": {"200": {"description": "a token", "content": {"application/json": {"schema": {"type": "object", "properties": {"access_token": {"type": "string"}, "expires_in": {"type": "integer"}}}}}}, "403": {"description": "not the appliance"}, "404": {"description": "no such drawer, or not linked"}, "502": {"description": "the provider or the gateway refused"}}}},
    "/api/projects/{name}/library": {"get": {"summary": "The house store's shelves: a folder per system (ES-DE's names), how many titles, how big.", "description": "What every seat reads. ?system=<shelf> lists that shelf's titles instead. 'ready' false = the store is away (the mount does not answer): the page says so and a push is refused.", "parameters": [{"name": "name", "in": "path", "required": true, "schema": {"type": "string"}}, {"name": "system", "in": "query", "required": false, "schema": {"type": "string"}}], "responses": {"200": {"description": "the shelves (or one shelf's titles)", "content": {"application/json": {"schema": {"type": "object", "properties": {"ready": {"type": "boolean"}, "why": {"type": "string"}, "path": {"type": "string"}, "shelves": {"type": "array", "items": {"$ref": "#/components/schemas/Shelf"}}, "systems": {"type": "array", "items": {"$ref": "#/components/schemas/System"}}, "titles": {"type": "array", "items": {"type": "object", "properties": {"name": {"type": "string"}, "bytes": {"type": "integer"}, "since": {"type": "string", "format": "date-time"}}}}}}}}}, "404": {"description": "no such project, or no library on it"}, "503": {"description": "the store is away"}}}, "post": {"summary": "Push a title onto its shelf — admins. multipart/form-data, streamed: never buffered, never overwritten.", "description": "Parts: 'system' (a shelf name; empty or 'auto' = from each file's own name — the store's own shelf that takes it, else the classic system for that extension; a disc image or an archive cannot tell and is refused until named) and 'file' (one or many — a cue and its bins travel together). Every file answers for itself: landed, or refused with the reason (already on the shelf, a name the shelf will not take, the wrong kind of file for that shelf). A file lands under a hidden name and is renamed into place once whole, so a seat never scans half a ROM. Logged with the person; counted.", "requestBody": {"required": true, "content": {"multipart/form-data": {"schema": {"type": "object", "properties": {"system": {"type": "string"}, "file": {"type": "array", "items": {"type": "string", "format": "binary"}}}}}}}, "parameters": [{"name": "name", "in": "path", "required": true, "schema": {"type": "string"}}], "responses": {"201": {"description": "at least one title landed", "content": {"application/json": {"schema": {"$ref": "#/components/schemas/Push"}}}}, "409": {"description": "nothing landed — every file refused, each with its reason", "content": {"application/json": {"schema": {"$ref": "#/components/schemas/Push"}}}}, "400": {"description": "not a multipart push, or no file in it"}, "403": {"description": "not an admin"}, "404": {"description": "no such project, or no library on it"}, "503": {"description": "the store is away"}}}},
    "/api/projects/{name}/library/detect": {"get": {"summary": "Which shelf a file name belongs on — asked before uploading anything.", "parameters": [{"name": "name", "in": "path", "required": true, "schema": {"type": "string"}}, {"name": "file", "in": "query", "required": true, "schema": {"type": "string"}}], "responses": {"200": {"description": "the shelf, or the candidates when more than one takes it ('error' says so; pick one)", "content": {"application/json": {"schema": {"type": "object", "properties": {"file": {"type": "string"}, "system": {"type": "string"}, "candidates": {"type": "array", "items": {"type": "string"}}, "error": {"type": "string"}}}}}}, "404": {"description": "no shelf takes that kind of file"}}}},
    "/links/{name}/{sidecar}/start": {"get": {"summary": "Link an account to a drawer — a person's tap, not for automations.", "description": "Sends the person to the provider (Authorization Code with PKCE, the house's own app, no secret). 'for' names the drawer: your own; an admin, anybody's or a shared one. The provider sends the person back to /links/callback; the code is traded for tokens and the refresh token — the grant — is stored with the person at the identity gateway (a shared drawer's with the panel). Linked from then on, from anywhere.", "parameters": [{"name": "name", "in": "path", "required": true, "schema": {"type": "string"}}, {"name": "sidecar", "in": "path", "required": true, "schema": {"type": "string"}}, {"name": "for", "in": "query", "required": false, "schema": {"type": "string"}}], "responses": {"302": {"description": "to the provider"}, "403": {"description": "not your drawer"}, "404": {"description": "nothing of that name to link"}}}},
  ...
  "x-reserved": {
    "description": "Declared, not implemented. Where the panel is going: saves; a person's own shelf. (Live sessions landed as /seats; external accounts as /links; the house store as /library.)",
    "paths": ["/api/projects/{name}/saves"]
  }
}
`))
}
