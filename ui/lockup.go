...
// substituted here, so the same binary shows "DEJARIK" to a stranger and
...
func base() []byte {
	once.Do(func() {
		b, err := files.ReadFile("static/logo-animated.svg")
		if err != nil {
			panic(err)
		}
		raw = b
...
func Lockup(house string) []byte {
	...
	out = reAria.ReplaceAll(out,
		[]byte(`aria-label="`+html.EscapeString(house)+` — dejarik"`))
	return out
}
